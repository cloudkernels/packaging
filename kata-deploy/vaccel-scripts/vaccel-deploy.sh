...
# For now we support nvidia jetson. This will be a table in the future.
vaccel_backend="jetson"
...
#!/bin/bash
VACCEL_BACKENDS=${VACCEL_BACKENDS} VACCEL_IMAGENET_NETWORKS=${vaccel_path}/share/data/networks CUDA_CACHE_PATH=$CUDA_CACHE_PATH} KATA_CONF_FILE=/opt/kata/share/defaults/kata-containers/configuration-${shim}.toml /opt/kata/bin/containerd-shim-kata-v2 \$@
EOT
	chmod +x "$shim_file"
